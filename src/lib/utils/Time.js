...
export default class Time {
    ...
    static buildWeeks(month, firstweekday, year) {
        ...
        if (firstweekday > 0) {
            let previousMonth =  moment().month(month).subtract(1, 'month').month()
            let previousYear = month === 0 ? year - 1 : year
            let daysInPreviousMonth = this.getDaysInMonth(previousMonth, previousYear)
            let prevWeekDay = 0
        ...
        let nextMonth = moment().month(month).add(1, 'month').month()
        let nextYear = month < 11 ? year : year + 1
        let nextMonthFirstWeekday = days[days.length - 1][0] + 1
        let nextNumberday = 1
        for (let i = nextMonthFirstWeekday; i < 7; i++) {
            nextDays.push([ nextMonthFirstWeekday++, nextNumberday++, nextMonth, nextYear ])
        }
    ...
    static getNextMonthWeeks(date) {
        ...
        let nextMonth = moment().month(date.month).add(1, 'month').month()
    ...
    static getLastMonthWeeks(date) {
        ...
        let previousMonth =  moment().month(date.month).subtract(1, 'month').month()
        let daysInMonth = this.getDaysInMonth(previousMonth, year)

        let firstweekday = this.getFistWeekdayOfMonth(date.days[0][0] - 1, daysInMonth)

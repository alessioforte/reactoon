...
class SliderRange extends Component {
  constructor(props) {
    ...
    const left = Math.round(
      ((minValue - this.min) * (this.width - this.height)) / this.range
    );
    const right = Math.round(
      ((this.max - maxValue) * (this.width - this.height)) / this.range
    );
  ...
  onMouseMoveRight(e) {
    ...
    let { left } = this.state,
      ...
      maxValue =
        this.max -
        Math.round((this.range / width) * rightOffset) +
        this.min -
        this.min,
      step = width / this.stepsCount;
  ...
  handleInput(e) {
    ...
    if (this.props.onChange)
      this.props.onChange({ min: value.minValue, max: value.maxValue });
  }

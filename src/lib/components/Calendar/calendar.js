...
class Calendar extends Component {
    ...
    handleOnScroll(e) {
        ...
        if (scrollLeft < 1) {
            ...
            this.calendar.current.scrollLeft = (previousMonth.days.length * this.dayWidth)

            this.setState({ months, endScroll: (daysCount * this.dayWidth) - this.width })
        }
        ...
        if (scrollLeft > endScroll - 1) {
            ...
            this.calendar.current.scrollLeft = ((daysCount - nextMonth.days.length) * this.dayWidth) - this.width
    ...
    render() {
        ...
        return (
            <Box width={this.width}>
                <ScrollBox ref={this.calendar} onScroll={this.handleOnScroll} width={this.width}>
                    {   
                        months.map((date, i) => (
                            <MonthBox key={`${date.year}-${date.month}`}>
                                <Header><Year>{date.year} </Year><span>{moment().month(date.month).format('MMMM')}</span></Header>

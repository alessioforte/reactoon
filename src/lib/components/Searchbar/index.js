...
const Searchbar = ({ suggestions, onChange, onSearch, onClear, delay, theme }) => {
    ...
    return (
        <ThemeProvider theme={theme}>
            <Bar ref={searchbar}>
                ...
                />
                { showClear && <Clear onClick={clearInput}><Delete size={8} color={theme.colors[getContrastYIQ(theme.colors.ground)]} /></Clear> }
                <Submit type="submit" onClick={handleSubmit}><SearchIcon size={15} color={theme.colors[getContrastYIQ(theme.colors.primary)]} /></Submit>
                {
                    showTips && (
                        <Box>
                            <List>
...
const Tip = styled.li`
    ...
    color: ${props.theme.colors.text};` : null}
    &:hover {
